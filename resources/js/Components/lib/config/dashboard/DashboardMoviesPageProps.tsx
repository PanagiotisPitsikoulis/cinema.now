...
import {DashboardMoviesData, DashboardPageProps} from "@/types/dashboard-types";
import {Key, useCallback, useMemo} from "react";
import {Movie} from "@/types/types";
import {createMovie, deleteMovie, editMovie, fetchMovies} from "@/Components/lib/api/dashboard/movie";
...
export function generateDashboardMoviesPageProps({
    ...
    const createForm = {
        schema: {
            fields: [
                {label: "Name", name: "name" as keyof Movie, type: "text", required: true},
                {label: "Description", name: "description" as keyof Movie, type: "textarea"},
                {label: "Image Link", name: "image_link" as keyof Movie, type: "text"},
                {label: "Category", name: "category" as keyof Movie, type: "text"},
    ...
    const onDeleteItem = async (item: Movie) => {
        try {
            ...
            setDisplayedItems((prev) => prev.filter((movie) => movie.id !== item.id));
            setSelectedId(null);
        } catch (error) {
