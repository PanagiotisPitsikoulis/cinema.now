 ...
 * @returns JSX.Element
 */
export default function Movies({auth, movies}: PageProps<{ movies: Movie[] }>) {
    ...
    const loadMoreMovies = async () => {
        const currentPage = Math.ceil(displayedMovies.length / ITEMS_PER_PAGE) + 1;

        try {
            const {movies: newMovies, hasMore: moreAvailable} = await handleFetchMoreMovies(
                currentPage,
                ITEMS_PER_PAGE
            );
